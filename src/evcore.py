...
SpecialKeyNames = set([_f for _f in """
...
""".split() if _f])
KnownEvents = set(list(SpecialKeyNames) + [_f for _f in """
...
""".split() if _f] + ["btn%d" % i for i in range(1, 20)])
...
def EventHelp():
    ...
    maxalen = max(list(map(len, KnownActions)))
    ...
    maxelen = max(list(map(len, EventMap)))
